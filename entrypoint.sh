...
# Install GitHub CLI at runtime (like Cursor CLI), skip if already present
...
# Install Glab at runtime (like Cursor CLI), skip if already present
if ! command -v glab >/dev/null 2>&1; then
  curl -sSL "https://raw.githubusercontent.com/upciti/wakemeops/main/assets/install_repository" | bash
  apt-get update -qq && apt-get install -y --no-install-recommends glab
  rm -rf /var/lib/apt/lists/*
fi
